...
def basket_kb(gid: str, hits: int):
    rows = []
    rows.append([InlineKeyboardButton(text="🏀 Бросить", callback_data=f"basket:{gid}:shoot", style="primary")])
    if hits > 0:
        ...
        if action == "shoot":
            ...
            if random.random() < HIT_CHANCE:
                ...
                if game["hits"] >= MAX_SHOTS:
                    ...
                    await query.message.edit_text(
                        f"🏀 <b>5/5 — PERFECT!</b>\n\n"
                        f"{E.WIN} <b>МАКСИМАЛЬНЫЙ ВЫИГРЫШ!</b>\n"
                        f"📈 Множитель: <b>x{mult}</b>\n"
                ...
                await query.message.edit_text(basket_text(game), reply_markup=basket_kb(gid, game["hits"]), parse_mode="HTML")
