...
async def legacy_tower_start(message: Message, bot: Bot):
    ...
    if len(parts) not in (2, 3):
        return await message.answer("Формат: <code>башня 0.5</code> или <code>башня 0.5 2</code> (мины 1-4)", parse_mode="HTML")
    ...
    async with _game_lock(user_id):
        ...
        gid = _new_gid("t")
        game = {
            "gid": gid, "uid": user_id, "bet": float(bet), "mines": int(mines),
...
async def legacy_tower_cb(query: CallbackQuery):
    ...
    async with _game_lock(query.from_user.id):
        ...
        if action == "cancel":
            ...
            bet = float(game["bet"])
            balance = await add_balance(query.from_user.id, bet)
        ...
        if action == "collect":
            ...
            payout = round(float(game["bet"]) * mult, 4)
