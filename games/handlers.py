...
async def check_sub_callback(call: CallbackQuery, bot: Bot):
    if await check_subscription(bot, call.from_user.id):
        # Рефералка — начисляем бонус
        ref = await get_referral(call.from_user.id)
        if ref and ref[3] == 0:
            referrer_id = ref[1]
            await update_balance(referrer_id, 0.1)
            await mark_bonus_paid(call.from_user.id)
            try:
                await bot.send_message(
                    referrer_id,
                    f"{E.BONUS} <b>+0.1 USDT за реферала!</b>\n"
                    f"👤 @{call.from_user.username or call.from_user.id} подписался на канал.",
                    parse_mode="HTML"
                )
            except Exception:
                pass
...
async def cmd_balance(message: Message, bot: Bot):
    await cmd_b_balance(message, bot)
...
async def game_tower_cb(call: CallbackQuery):
    await call.message.edit_text(
        ...
        f"2️⃣ Или: <code>башня 0.5 2</code> — 2 мины\n\n"
        f"🎯 Пройди 9 этажей, не попав на мину.\n"
...
async def game_gold_cb(call: CallbackQuery):
    await call.message.edit_text(
        ...
        f"2️⃣ Выбирай сторону: ◀️ или ▶️\n"
        f"3️⃣ Одна из сторон — ловушка.\n\n"
        f"💰 Множители: 1.5x → 2x → ... → 18x\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def game_dice_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"🎲 <b>КУБИК — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>куб 0.5 6</code>\n"
        f"2️⃣ Ставка 0.5, число 6\n"
        f"3️⃣ Бот кинет кубик 1-6\n"
        f"4️⃣ Угадал — выигрыш <b>x6</b>\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def game_kwak_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"🐸 <b>КВАК — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>квак 0.5</code>\n"
        f"2️⃣ Жаба прыгает по кувшинкам\n"
        f"3️⃣ Каждый прыжок — шанс упасть 30%\n\n"
        f"💰 Множители: x1.4 → x2 → ... → x30\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def game_bowling_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"🎳 <b>БОУЛИНГ — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>боулинг 0.5 7</code>\n"
        f"2️⃣ Ставка 0.5, прогноз 7 кеглей\n"
        f"3️⃣ Если угадал — выигрыш по множителю\n\n"
        f"💰 Множители: 6→x3, 7→x4, 8→x6, 9→x10, 10→x20\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def game_football_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"⚽ <b>ФУТБОЛ — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>футбол 0.5</code>\n"
        f"2️⃣ Выбери зону удара\n"
        f"3️⃣ Вратарь прыгнет в одну из зон\n"
        f"4️⃣ Не совпало — гол!\n\n"
        f"📈 Множитель: <b>x2.7</b>\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def game_basketball_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"🏀 <b>БАСКЕТБОЛ — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>баскетбол 0.5</code>\n"
        f"2️⃣ Бросай (шанс 45%)\n"
        f"3️⃣ 5 бросков максимум\n\n"
        f"💰 Множители: 1→x1.5, 2→x2.5, 3→x4, 4→x7, 5→x15\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def game_slots_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"🎰 <b>СЛОТЫ — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>слоты 0.5</code>\n"
        f"2️⃣ Крути барабаны\n"
        f"3️⃣ 3 одинаковых = выигрыш\n\n"
        f"💰 🍒x3 | 🍋x4 | 🍇x6 | 💎x10 | 7️⃣x25\n\n"
        f"👇 Напиши команду в чат:",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="menu_games", style="danger")],
...
async def menu_bonus(call: CallbackQuery, bot: Bot):
    ...
    variants_block = (
        f"📋 <b>Выбери один из вариантов</b> для bio:\n"
        f"<blockquote expandable>{variants_lines}</blockquote>\n"
        f"<i>Нажми на блок выше, чтобы развернуть</i>"
    )
    ...
    if not has_text:
        text = (
            ...
            f"1. Поставь <b>одну из фраз</b> в <b>описании профиля</b> (bio)\n"
            f"2. Вернись сюда и нажми <b>«✅ Проверить»</b>\n\n"
            f"{variants_block}\n\n"
            f"🎁 Награда: <b>0.55 USDT</b> за <b>7️⃣ 7️⃣ 7️⃣</b>"
        )
    ...
    if not can_spin:
        text = (
            f"{E.BONUS} <b>Ежедневный бонус</b>\n\n"
            f"{E.SUCCESS} Bio подтверждён!\n"
            f"Ты уже крутил сегодня.{wait_text}"
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="❌ Назад", callback_data="back_main", style="danger")],
        ])
    ...
    text = (
        f"{E.BONUS} <b>Ежедневный бонус</b>\n\n"
        f"{E.SUCCESS} Bio подтверждён!\n"
        f"🎰 Угадай <b>7️⃣ 7️⃣ 7️⃣</b> и получи <b>0.55 USDT</b>!\n\n"
        f"👇 Крути!"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def bonus_check(call: CallbackQuery, bot: Bot):
    ...
    if not has_text:
        await call.answer(
            "❌ Bio не подтверждён.\nДобавь одну из фраз в описание профиля.",
            show_alert=True
        )
        return
...
async def bonus_spin(call: CallbackQuery, bot: Bot):
    ...
    if bonus and bonus[2]:
        try:
            ...
            if datetime.utcnow() - last < timedelta(hours=24):
                await call.answer("⏳ Уже крутил сегодня", show_alert=True)
                return
        except Exception:
            pass
    ...
    if reels[0] == reels[1] == reels[2] == "7️⃣":
        ...
        await call.message.answer(
            ...
            f"💰 Начислено: <b>+{BONUS_AMOUNT} USDT</b>\n"
            f"{E.BALANCE} Баланс: <b>{user[2]:.2f}</b>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🏠 Главное меню", callback_data="back_main", style="primary")],
            ])
        )
    else:
        await call.message.answer(
            ...
            f"➖➖➖➖➖➖➖➖➖\n"
            f"🎰 Выпало: <b>{display}</b>\n"
            f"Нужно: <b>7️⃣ 7️⃣ 7️⃣</b>\n\n"
            f"{E.WAIT} Следующая попытка через 24 часа",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="🏠 Главное меню", callback_data="back_main", style="primary")],
            ])
...
async def menu_my_checks(call: CallbackQuery):
    ...
    if not checks:
        await call.message.edit_text(
            f"{E.HISTORY} <b>У тебя пока нет чеков.</b>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        ...
        return
    text = f"{E.HISTORY} <b>Твои чеки</b>\n\nВсего: <b>{len(checks)}</b>"
    try:
...
async def mycheck_view(call: CallbackQuery, bot: Bot):
    ...
    if creator_id != call.from_user.id:
        await call.answer("Это не твой чек", show_alert=True)
        return
    ...
    text = (
        ...
        f"{E.ACTIVATIONS} Активаций: <b>{act}/{max_act}</b>\n"
        f"👤 Получатель: {('@' + target_username) if target_username else 'Публичный'}\n"
        f"📅 Статус: {'🟢 Активен' if status == 'active' else '⚪ Завершён'}"
    )
    try:
...
async def mycheck_del_yes(call: CallbackQuery, bot: Bot):
    ...
    if creator_id != call.from_user.id:
        ...
        return
    if status != "active":
        await call.answer("Чек уже завершён", show_alert=True)
        return
    ...
    await call.message.edit_text(
        f"{E.SUCCESS} <b>Чек удалён</b>\n\n"
        f"💰 Возврат: <code>{refund:.2f}</code> {asset}\n"
        f"Остаток: <b>{remaining}</b> активаций",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def transfer_target(message: Message, state: FSMContext, bot: Bot):
    ...
    if target_text.startswith("@"):
        ...
        try:
            ...
            await message.answer(
                f"{E.ERROR} Пользователь не найден.\nПопроси его запустить @Stake_pay_bot.",
                parse_mode="HTML", reply_markup=cancel_kb()
            )
            return
    else:
...
async def menu_help(call: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        f"{E.BALANCE} Баланс\n{E.CHECK} Создать чек\n"
        f"{E.DEPOSIT} Пополнение (Stars / Крипта)\n"
        f"{E.WITHDRAW} Вывод\n"
        f"{E.GAMES} Игры\n"
        f"💸 Перевести\n{E.BONUS} Бонус\n"
