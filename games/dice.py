...
def dice_number_kb(gid: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="❌ Назад", callback_data=f"dice:{gid}:back", style="danger")],
    ])
...
async def dice_cb(query: CallbackQuery):
    ...
    async with _game_lock(query.from_user.id):
        ...
        if action in ("big", "small", "exact3"):
            ...
            if action == "big" and roll > 3:
                win = True
                mult = 1.5
            elif action == "small" and roll < 3:
                win = True
                mult = 1.5
            elif action == "exact3" and roll == 3:
                win = True
                mult = 3.0

            choice_label = {"big": "больше 3", "small": "меньше 3", "exact3": "ровно 3"}[action]
            ...
            if win:
                ...
                await query.message.answer(
                    ...
                    f"💰 Выигрыш: x{mult} / {fmt_money(payout)}\n"
                    f"➖➖➖➖➖➖➖➖➖\n"
                    f"⚡ Итог: <b>{roll}</b>\n"
        ...
        if action == "n":
            ...
            if win:
                mult = 4.0
                payout = round(bet * mult, 4)
                balance = await finalize_bet(query.from_user.id, bet, payout, "dice", f"num_win_{roll}")
                DICE_GAMES.pop(gid, None)
                await query.message.answer(
                    f"🔥 <b>Кубик · Победа!</b> ✅\n"
                    f"➖➖➖➖➖➖➖➖➖\n"
                    f"🎲 Выпало: <b>{DICE_EMOJI[roll]}</b>\n"
                    f"🎯 Выбрано: <b>{DICE_EMOJI[num]}</b>\n"
                    f"💰 Выигрыш: x{mult} / {fmt_money(payout)}\n"
                    f"➖➖➖➖➖➖➖➖➖\n"
                    f"⚡ Итог: <b>{roll}</b>\n"
                    f"{E.BALANCE} Баланс: <b>{fmt_money(balance)}</b>",
                ...
                balance = await finalize_bet(query.from_user.id, bet, 0.0, "dice", f"num_lose_{roll}")
                DICE_GAMES.pop(gid, None)
                await query.message.answer(
                    f"💔 <b>Кубик · Проигрыш</b>\n"
                    f"➖➖➖➖➖➖➖➖➖\n"
