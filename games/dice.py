...
def dice_multiplier(threshold: int) -> float:
    threshold = max(2, min(98, int(threshold)))
    p = threshold / 100.0
...
def dice_text(game: Dict[str, Any]) -> str:
    bet = float(game["bet"])
    threshold = int(game["threshold"])
    mult = dice_multiplier(threshold)
    potential = bet * mult

    return (
        f"🎲 <b>КОСТИ</b>\n\n"
        f"Ставка: <b>{fmt_money(bet)}</b>\n"
        f"Порог: <b>{threshold}</b> (выигрыш если < {threshold})\n"
        f"Множитель: <b>x{mult}</b>\n"
        f"Потенциальный выигрыш: <b>{fmt_money(potential)}</b>\n\n"
        f"👇 Нажми «Бросить»"
    )
...
async def dice_start(message: Message, bot: Bot):
    ...
    async with _game_lock(user_id):
        if any(g.get("uid") == user_id and g.get("state") == "playing" for g in DICE_GAMES.values()):
            return await message.answer("У тебя уже активная игра в кости.")
        ...
        if not (2 <= threshold <= 98):
            return await message.answer("Порог: 2-98")
        if bet < MIN_BET:
        ...
        gid = _new_gid("d")
        game = {
            "gid": gid, "uid": user_id, "bet": float(bet),
            "threshold": threshold, "state": "playing"
        }
        DICE_GAMES[gid] = game
    ...
    async with _game_lock(query.from_user.id):
        ...
        if action == "cancel":
            ...
            await query.message.edit_text(
                f"❌ Игра отменена. Возврат: <b>{fmt_money(bet)}</b>",
                parse_mode="HTML"
            )
            return await query.answer()
