import random
import asynciofrom typing import Dict, Any
    ...
    async with _game_lock(query.from_user.id):
        ...
        if action == "spin":
            ...
            if reels[0] == reels[1] == reels[2]:
                ...
                balance = await finalize_bet(query.from_user.id, bet, payout, "slots", f"win_{reels[0]}")
                SLOTS_GAMES.pop(gid, None)
                ...
                await query.message.answer(
                    ...
                    f"➖➖➖➖➖➖➖➖➖\n"
                    f"🎰 Выпало: <b>{display}</b>\n"
                    f"{E.RATES} Множитель: x{TWO_MATCH}\n"
                    f"💰 Выигрыш: {fmt_money(payout)}\n"
                    f"➖➖➖➖➖➖➖➖➖\n"
                    f"{E.BALANCE} Баланс: <b>{fmt_money(balance)}</b>",
                    parse_mode="HTML"
