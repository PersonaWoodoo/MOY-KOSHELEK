...
def main_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            InlineKeyboardButton(text="🎁 Бонус", callback_data="menu_bonus", style="success"),
            InlineKeyboardButton(text="🏆 Топ", callback_data="menu_top", style="success"),
        ],
        [
...
def games_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎲 Кубик", callback_data="game_dice", style="primary")],
        [InlineKeyboardButton(text="🐸 Квак", callback_data="game_kwak", style="primary")],
        [InlineKeyboardButton(text="🎳 Боулинг", callback_data="game_bowling", style="primary")],
        [InlineKeyboardButton(text="⚽ Футбол", callback_data="game_football", style="primary")],
        [InlineKeyboardButton(text="🏀 Баскетбол", callback_data="game_basketball", style="primary")],
        [InlineKeyboardButton(text="🎰 Слоты", callback_data="game_slots", style="primary")],
...
def deposit_methods_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="⭐ Звёзды", callback_data="dep_stars", style="success")],
        [InlineKeyboardButton(text="💎 Крипта (ручная заявка)", callback_data="dep_crypto", style="primary")],
...
def withdraw_network_menu(gid: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="TON", callback_data=f"wdnet:{gid}:TON", style="primary")],
        [InlineKeyboardButton(text="TRC20 (USDT)", callback_data=f"wdnet:{gid}:TRC20", style="primary")],
        [InlineKeyboardButton(text="BEP20 (USDT)", callback_data=f"wdnet:{gid}:BEP20", style="primary")],
        [InlineKeyboardButton(text="ERC20 (USDT)", callback_data=f"wdnet:{gid}:ERC20", style="primary")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data=f"wdnet:{gid}:cancel", style="danger")],
    ])
...
def check_type_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Публичный", callback_data="chktype_public", style="primary")],
        [InlineKeyboardButton(text="🚫 С паролем", callback_data="chktype_password", style="primary")],
        [InlineKeyboardButton(text="🎁 Для рефералов", callback_data="chktype_refs", style="primary")],
        [InlineKeyboardButton(text="📈 По обороту", callback_data="chktype_turnover", style="primary")],
        [InlineKeyboardButton(text="❌ Отмена", callback_data="back_main", style="danger")],
    ])


def check_share_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📤 Отправить другу", switch_inline_query="", style="success")],
        [InlineKeyboardButton(text="🔗 Скопировать", callback_data="check_copy_link", style="primary")],
        [InlineKeyboardButton(text="⬅️ В меню", callback_data="back_main", style="primary")],
    ])


def check_password_menu(code: str):
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🚫 Ввести пароль", callback_data=f"check_pass_input:{code}", style="primary")],
        [InlineKeyboardButton(text="⬅️ В меню", callback_data="back_main", style="primary")],
    ])
...
def my_check_actions_menu(cid: int, code: str, can_delete: bool = True, can_share: bool = True):
    ...
    if can_share:
        ...
        rows.append([InlineKeyboardButton(text="📤 Поделиться", url=share_url, style="success")])
    rows.append([InlineKeyboardButton(text="🔗 Ссылка", callback_data=f"mycheck_copy_{cid}", style="primary")])
    if can_delete:
        rows.append([InlineKeyboardButton(text="❌ Удалить", callback_data=f"mycheck_del_{cid}", style="danger")])
...
# ==================== РЕФЕРАЛКА ====================
def referrals_menu(ref_link: str):
    share_url = f"https://t.me/share/url?url={ref_link}&text=Залетай в Stake Pay!"
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="📤 Поделиться", url=share_url, style="success")],
        [InlineKeyboardButton(text="🔗 Ссылка", callback_data="ref_copy", style="primary")],
        [InlineKeyboardButton(text="❌ Назад", callback_data="back_main", style="danger")],
    ])
...
def admin_menu():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📥 Заявки на пополнение", callback_data="admin_deposits", style="primary")],
...
def withdrawal_action_menu(wid: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Выплачено", callback_data=f"admin_wd_paid:{wid}", style="success"),
            InlineKeyboardButton(text="❌ Отказано", callback_data=f"admin_wd_reject:{wid}", style="danger"),
        ],
    ])
...
def deposit_action_menu(did: int):
    return InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Зачислить", callback_data=f"admin_dep_ok:{did}", style="success"),
            InlineKeyboardButton(text="❌ Отказать", callback_data=f"admin_dep_reject:{did}", style="danger"),
        ],
    ])
