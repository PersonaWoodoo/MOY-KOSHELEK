...
async def get_check(check_id):
    def _get():
        conn = sqlite3.connect(DB_PATH)
        cur = conn.execute("SELECT * FROM checks WHERE check_id = ?", (check_id,))
        row = cur.fetchone()
        conn.close()
        return row
...
async def activate_check(check_id):
    def _act():
        conn = sqlite3.connect(DB_PATH)
        conn.execute("UPDATE checks SET status = 'activated' WHERE check_id = ?", (check_id,))
        conn.commit()
        conn.close()
...
async def get_stats():
    def _stats():
        conn = sqlite3.connect(DB_PATH)
        cur = conn.execute("SELECT COUNT(*), SUM(amount) FROM checks WHERE status = 'activated'")
        row = cur.fetchone()
        conn.close()
        return row
