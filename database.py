...
def _init_db():
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS own_checks (
            ...
            password TEXT,
            check_type TEXT DEFAULT 'public',
...
def _update_turnover(user_id, amount):
    conn = sqlite3.connect(DB_PATH)
    conn.execute("UPDATE users SET turnover = turnover + ? WHERE user_id = ?", (amount, user_id))
    conn.commit()
    conn.close()
...
def _get_top_users(limit=10):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.execute(
        "SELECT user_id, username, balance FROM users WHERE is_banned = 0 ORDER BY balance DESC LIMIT ?",
        (limit,)
    )
    rows = cur.fetchall()
    conn.close()
...
# ========== CHECKS ==========
def _create_own_check_v2(code, creator_id, target_id, target_username, asset, amount, description, max_activations, password="", check_type="public", min_turnover=0):
    conn = sqlite3.connect(DB_PATH)
    conn.execute(
        "INSERT INTO own_checks (code, creator_id, target_id, target_username, asset, amount, description, max_activations, password, check_type, min_turnover) "
        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        (code, creator_id, target_id, target_username, asset, amount, description, max_activations, password, check_type, min_turnover)
    )
    conn.commit()
    conn.close()
...
def _set_withdrawal_status(wid, status, reason=""):
    conn = sqlite3.connect(DB_PATH)
    conn.execute("UPDATE withdrawals SET status = ?, reject_reason = ? WHERE id = ?", (status, reason, wid))
    conn.commit()
    conn.close()
...
# ========== PAID INVOICES (для polling) ==========
def _save_invoice(invoice_id, user_id, amount, asset):
    conn = sqlite3.connect(DB_PATH)
    try:
        conn.execute(
            "INSERT INTO paid_invoices (invoice_id, user_id, amount, asset) VALUES (?, ?, ?, ?)",
            (invoice_id, user_id, amount, asset)
        )
        conn.commit()
        return True
    except sqlite3.IntegrityError:
...
def _get_pending_invoice(invoice_id):
    conn = sqlite3.connect(DB_PATH)
    cur = conn.execute("SELECT * FROM paid_invoices WHERE invoice_id = ?", (invoice_id,))
    row = cur.fetchone()
    conn.close()
    return row


def _mark_invoice_paid(invoice_id, status="paid"):
    conn = sqlite3.connect(DB_PATH)
    conn.execute("UPDATE paid_invoices SET status = ? WHERE invoice_id = ?", (status, invoice_id))
    conn.commit()
    conn.close()
...
async def update_turnover(user_id, amount):
    await asyncio.to_thread(_update_turnover, user_id, amount)
...
async def get_top_users(limit=10):
    return await asyncio.to_thread(_get_top_users, limit)
...
async def create_own_check_v2(code, creator_id, target_id, target_username, asset, amount, description, max_activations, password="", check_type="public", min_turnover=0):
    await asyncio.to_thread(
        _create_own_check_v2, code, creator_id, target_id, target_username,
        asset, amount, description, max_activations, password, check_type, min_turnover
    )
...
async def save_invoice(invoice_id, user_id, amount, asset):
    return await asyncio.to_thread(_save_invoice, invoice_id, user_id, amount, asset)


async def get_pending_invoice(invoice_id):
    return await asyncio.to_thread(_get_pending_invoice, invoice_id)


async def mark_invoice_paid(invoice_id, status="paid"):
    await asyncio.to_thread(_mark_invoice_paid, invoice_id, status)
...
async def create_own_check(code, creator_id, target_id, target_username, asset, amount, description, max_activations=1):
    await create_own_check_v2(code, creator_id, target_id, target_username, asset, amount, description, max_activations)
