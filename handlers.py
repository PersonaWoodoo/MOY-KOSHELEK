...
from states import CheckStates, DepositStates, WithdrawStates, TransferStates, AdminStates
...
# ========== АКТИВАЦИЯ ЧЕКА (жёсткая проверка подписки) ==========
...
async def wd_asset(call: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    limits = {
        "STARS": f"Минимум: {MIN_WITHDRAW_STARS} ⭐",
...
async def wd_amount(message: Message, state: FSMContext, bot: Bot):
    ...
    if asset == "STARS" and amount < MIN_WITHDRAW_STARS:
        await message.answer(
            f"{E.ERROR} Минимум для вывода звёзд: <b>{MIN_WITHDRAW_STARS} ⭐</b>",
            parse_mode="HTML", reply_markup=cancel_kb()
        )
        return
