...
async def game_gold_cb(call: CallbackQuery):
    await call.message.edit_text(
        ...
        f"2️⃣ На каждом уровне выбирай: ◀️ или ▶️\n"
...
async def game_dice_cb(call: CallbackQuery):
    await call.message.edit_text(
        f"🎲 <b>КОСТИ — как играть</b>\n\n"
        f"1️⃣ Отправь: <code>кости 0.5 50</code>\n"
        f"2️⃣ Ставка 0.5, порог 50\n"
        f"3️⃣ Бот кинет число 1-100\n"
        f"4️⃣ Если число < 50 — победа\n\n"
        f"💰 Множители:\n"
        f"• Порог 50 → x1.98\n"
        f"• Порог 20 → x4.95\n"
        f"• Порог 10 → x9.90\n\n"
...
async def mycheck_view(call: CallbackQuery, bot: Bot):
    ...
    try:
        await call.message.edit_text(
            ...
            reply_markup=my_check_actions_menu(cid, code, can_delete=can_delete)
        )
    except Exception:
        await call.message.answer(
            text, parse_mode="HTML",
...
async def wd_amount(message: Message, state: FSMContext, bot: Bot):
    ...
    if amount < MIN_WITHDRAW_CRYPTO:
        await message.answer(
            f"{E.ERROR} Минимум для вывода: <b>${MIN_WITHDRAW_CRYPTO}</b>",
            parse_mode="HTML", reply_markup=cancel_kb()
        )
        return
...
async def menu_help(call: CallbackQuery, bot: Bot):
    ...
    text = (
        ...
        f"{E.WITHDRAW} Вывод\n"
        f"{E.GAMES} Игры (Башня, Золото, Кости)\n"
