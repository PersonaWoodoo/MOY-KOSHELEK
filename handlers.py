...
async def cmd_start(message: Message, bot: Bot):
    ...
    if user and user[3] == 1:
        await message.answer(f"{E.ERROR} Ты забанен.")
        return
...
async def menu_balance(call: CallbackQuery, bot: Bot):
    ...
    await call.message.edit_text(
        ...
        f"{E.STARS} Stars: <code>{int(rub / STAR_RATE)}</code> (по курсу {STAR_RATE}₽)",
        parse_mode="HTML",
...
async def dep_rub_amount(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"После перевода отправь <b>ID транзакции</b> (номер операции из банка).",
        parse_mode="HTML", reply_markup=cancel_kb()
    )
    ...
    for admin_id in ADMIN_IDS:
        try:
            await bot.send_message(
                admin_id,
                f"💰 <b>Заявка на пополнение RUB</b>\n\n"
...
# ----- STARS (официальный платёж Telegram) -----
...
async def dep_crypto_amount(message: Message, state: FSMContext, bot: Bot):
    ...
    if asset == "STARS":
        # Официальный счёт Telegram Stars — отправляем пользователю инвойс
        try:
...
async def check_target(message: Message, state: FSMContext, bot: Bot):
    ...
    if target_text != "-":
        ...
        try:
            ...
            await message.answer(f"{E.ERROR} Не найден. Попробуй ещё или <code>-</code>.", parse_mode="HTML", reply_markup=cancel_kb())
            return

    result = await create_check(
        asset=data["asset"], amount=data["amount"], description=data["description"]
    )
    if not result.get("ok"):
        await message.answer(f"{E.ERROR} {result}", parse_mode="HTML", reply_markup=main_menu())
        await state.clear()
        return

    cd = result["result"]
    await save_check(cd["check_id"], message.from_user.id, target_id, data["asset"], data["amount"], data["description"])
    await update_balance(message.from_user.id, -data["amount"])

    url = cd.get("bot_check_url", "")
    await message.answer(
        f"{E.SUCCESS} <b>Чек создан!</b>\n\n"
        f"💎 {data['asset']}: <code>{data['amount']}</code>\n"
        f"📝 {data['description'] or '—'}\n"
        f"{E.PERSONAL} {'@' + target_text if target_id else 'Публичный'}\n\n"
        f"🔗 {url}",
        parse_mode="HTML", reply_markup=main_menu()
    )
    ...
    if target_id:
        try:
            await bot.send_message(target_id, f"🎁 Чек на <code>{data['amount']} {data['asset']}</code>\n🔗 {url}", parse_mode="HTML")
        except Exception:
            pass
    await state.clear()
...
async def admin_panel(message: Message):
    if not is_admin(message.from_user.id):
        ...
        return
    await message.answer("🛠 <b>Админ-панель Stake Pay</b>", parse_mode="HTML", reply_markup=admin_menu())
...
async def admin_stats(call: CallbackQuery):
    ...
    await call.message.edit_text(
        f"📊 <b>Статистика</b>\n\n"
        f"👥 Юзеров: <code>{len(users)}</code>\n"
        f"✅ Чеков: <code>{count or 0}</code>\n"
...
async def give_user(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"👤 ID: <code>{uid}</code>\n\nВведи сумму (в USDT-эквиваленте):",
        parse_mode="HTML", reply_markup=cancel_kb()
    )
...
async def give_amount(message: Message, state: FSMContext, bot: Bot):
    ...
    try:
        await bot.send_message(target, f"🎁 Тебе начислено <code>{amount}</code>", parse_mode="HTML")
    except Exception:
        pass
    await state.clear()
...
async def admin_broadcast(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "📢 <b>Рассылка</b>\n\nОтправь текст, который разослать всем:",
        parse_mode="HTML", reply_markup=cancel_kb()
    )
